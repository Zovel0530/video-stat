...
| 原来的问题 | 这个项目怎么解决 |
|-----------|----------------|
| 排行榜混在一起，看不出类型分布 | 把 B站 20+ 个分区归为 10 个大类，分别统计 |
...
B站的视频分区有 20 多个。分类器把它们归为 10 组，按匹配优先级排列：
...
影视娱乐   → 娱乐 + 影视 + 电影 + 电视剧 + 番剧 + 国创 + 综艺 + 明星 + 特摄 + 预告资讯
知识科技   → 知识 + 科技 + 纪录片 + 科学科普 + 计算机 + 极客DIY + 设计创意 + 人文历史 + 社科法律 + 心理 + 数码
...
194b5f2 更新了抖音的提示
2205703 细化了分类
44b0e7d 调整了动画顺序，解决了数据显示异常的问题
67429ab 解决了之前点击两次获取数据才能实现功能的bug
c1599e8 分类获取bug修复
610a570 修复30天数据抓取器异常的问题
1de74ea 修复了时间归档不一致的问题
1b11d02 完善
728646b readme编写
...
### 7.6 30 天数据获取返回"no data"

B站归档 API（`/popular/series/one`）需要登录态，HTTP 返回 200 但业务 code 为 `-352`。旧代码只检查 HTTP 状态码，看到 200 就当成功处理，结果 `res.data.list` 是 `undefined`，走到了空数组兜底，没有触发热门 API 的 fallback。

**处理**：给所有 B站 API 函数加了 `checkCode()` 校验——HTTP 200 但 `code !== 0` 时直接抛异常。30 天拉取逻辑也拆成了两条路径：先尝试归档 API，任何环节失败都自动回退到热门 API 乘 3 页 + 时间过滤。

### 7.7 统计卡片首次点击数据显示为 0，第二次点击才正常

`watch(() => props.videos, ...)` 开了 `immediate: true`。immediate 在 `setup()` 阶段就触发，此时 `container.value` 还是 `null`（template ref 还没绑定），动画函数里的 `if (!container.value) return` 直接跳过了。第二次点击时 DOM 已存在，动画正常。

**处理**：去掉 `immediate`。初始渲染交给 `onMounted`（此时 DOM 保证存在），后续数据变化交给 `watch`（不再带 immediate）。DataTable 组件的行飞入动画也是同样问题，用同样方式修复。

### 7.8 分类系统"其他"占比过大

旧分类只有 6 个大类，"搞笑""日常""三农""综合"等分区没有对应组，全掉进"其他"，导致饼图里"其他"占比过高，没有分析价值。

**处理**：分析未分类分区的视频数量，把排名靠前的 4 个分区抽出来独立成组。最终从 6 类扩展到 10 类：搞笑、日常、三农、综合、游戏、音乐舞蹈、动画、影视娱乐、知识科技、生活。每个类扩充了关键词（如动画组加入同人、手书、GMV、MMD），匹配优先级从具体到宽泛排列，先匹配到的胜出。100 样本测试匹配率 100%。

### 7.9 FilterPanel 语法错误导致按钮消失

在添加"抖音 (开发中)"按钮时，不小心用了中文冒号 `：` 代替英文逗号 `,`，导致 JS 解析失败，整个组件不渲染。

**处理**：将 `label: '抖音(开发中)' ：disabled: true` 改为 `label: '抖音 (开发中)', disabled: true`。

### 7.10 项目辅助文件分散在根目录

PaperSpine、awesome-design-md、GSAP skills 等文件散落在项目根目录，和业务代码混在一起，且会污染 git 提交。

**处理**：将 PaperSpine、awesome-design-md 和 `.agents/skills/` 全部迁移到 `.claude/` 目录下统一管理，并将 `.claude/` 加入 `.gitignore`。Claude Code 原生支持从 `.claude/skills/` 加载项目级技能，迁移后功能不变，仓库更干净。
