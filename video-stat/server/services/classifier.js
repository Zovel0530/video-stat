// Bilibili partition ID → name mapping
const PARTITION_MAP = {
...
export function classifyVideo(video) {
  ...
  const group = findCategoryGroup(tname)
  return { ...video, tname, categoryGroup: group }
}

function findCategoryGroup(tname) {
...
export function getCategoryStats(videos) {
  ...
  for (const v of videos) {
    ...
    if (!stats[cat]) {
      stats[cat] = { count: 0, view: 0, like: 0, reply: 0, danmaku: 0, share: 0 }
    }
    stats[cat].count++
